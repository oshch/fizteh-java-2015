package ru.fizteh.fivt.students.oshch.threads;
...
public class BlockingQueue<T> {
    ...
    public final void offer(List<T> newElements, long timeout) throws InterruptedException, TimeoutException {
        ...
        try {
            ...
            while (elements.size() + newElements.size() > maxSize && (timeout == 0 || nanos > 0)) {
                if (timeout == 0) {
                    notFull.await();
                } else {
    ...
    public final List<T> take(int count, long timeout) throws InterruptedException {
        ...
        try {
            ...
            long nanos = TimeUnit.MILLISECONDS.toNanos(timeout);
            while (elements.size() < count && (timeout == 0 || nanos > 0)) {
                if (timeout == 0) {
                    notEmpty.await();
                } else {
